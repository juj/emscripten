...
def run_js(filename, engine, args=None,
           ...
           timeout=DEFAULT_TIMEOUT, interesting=False):
  ...
  try:
    if interesting:
      stdout = sys.stdout#subprocess.STDOUT
      stderr = sys.stderr#subprocess.STDOUT
#      stderr = stdout
#      command = ['qemu-s390x', '-d', 'in_asm,cpu,int,op', '-D', '/home/clb/qemu.log'] + command[1:]
#      command = ['qemu-s390x', '-monitor', 'stdio'] + command[1:]
