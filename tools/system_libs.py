...
def calculate(temp_files, in_temp, stdout_, stderr_, forced=[]):
  ...
  def create_pthreads_mt(libname):
    # Add pthread files.
  ...
  def create_pthreads_st(libname):
    # Singlethreaded pthreads code (mostly stubs for APIs that are useful to call even when no threading is available)
  ...
  if shared.Settings.USE_PTHREADS:
    system_libs += [('libc-mt',                     'bc', create_libc,                           libc_symbols,     [],       False),
                    ('pthreads-mt',                 'bc', create_pthreads_mt,                    pthreads_symbols, ['libc'], False),
    ...
    system_libs += [('libc', 'bc', create_libc, libc_symbols, [], False),
                    ('pthreads-st',                  'bc', create_pthreads_st,                    pthreads_symbols, ['libc'], False)]
