...
class SettingsManager(object):
  class __impl(object):
    ...
    def target_environment_may_be(self, environment):
      SHELL_ENVIRONMENTS = ['spidermonkey', 'jsc', 'v8']
      targeted_environments = self.attrs['ENVIRONMENT'].split(',')
      return self.attrs['ENVIRONMENT'] == '' or environment in targeted_environments or (environment in SHELL_ENVIRONMENTS and 'shell' in targeted_environments)
...
class Building(object):
  ...
  def closure_compiler(filename, pretty=True):
    with ToolchainProfiler.profile_block('closure_compiler'):
      ...
      if not Settings.FILESYSTEM and not Settings.ASMFS:
      ...
      args = [JAVA,
              ...
              '--language_in', 'ECMASCRIPT5'] + CLOSURE_ANNOTATIONS + ['--externs', CLOSURE_EXTERNS,
      ...
      if Settings.target_environment_may_be('node'):
        for extern in NODE_EXTERNS:
          args.append('--externs')
          args.append(extern)
      if Settings.target_environment_may_be('v8'):
        for extern in V8_EXTERNS:
          args.append('--externs')
          args.append(extern)
      if Settings.target_environment_may_be('spidermonkey'):
        for extern in SPIDERMONKEY_EXTERNS:
          args.append('--externs')
          args.append(extern)
