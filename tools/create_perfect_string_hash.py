...
# Use #include <emscripten/key_codes.h> in your code to access these IDs.
...
print '''/* This file was automatically generated from script
tools/create_perfect_string_hash.py. Edit that file to make changes here.
Run

  python tools/create_perfect_string_hash.py > system/include/emscripten/dom_pk_codes.h
...
for s in input_strings:
  print '#define ' + pad_to_length(s[2], longest_dom_pk_code_length()) + ' 0x%04X /* "%s */' % (s[0], pad_to_length(s[1] + '"', longest_key_code_length()+1))

print '''
/* Maps the EmscriptenKeyboardEvent::code field from emscripten/html5.h to one of the DOM_PK codes above. */
static DOM_PK_CODE_TYPE emscripten_compute_dom_pk_code(const char *keyCodeString)
{
...
for s in input_strings:
  print '    case 0x%08XU /* %s */: return %s /* 0x%04X */' % (str_to_hash[s[1]], pad_to_length(s[1], longest_key_code_length()), pad_to_length(s[2] + ';', longest_dom_pk_code_length()+1), s[0])

print '''    default: return DOM_PK_UNKNOWN;
  }
}

/* Returns the string representation of the given key code ID. Useful for debug printing. */
static const char *emscripten_dom_pk_code_to_string(DOM_PK_CODE_TYPE code)
{
  switch(code)
  {'''
...
for s in input_strings:
  if len(s) == 3:
    print '    case %s return "%s";' % (pad_to_length(s[2] + ':', longest_dom_pk_code_length()+1), s[2])

print '''    default: return "Unknown DOM_PK code";
  }
}
'''
