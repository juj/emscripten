...
int main()
{
	...
	uint8_t prevUchar = emscripten_atomic_add_u8(&globalUchar, 1); assert(prevUchar == 4);
	uint16_t prevUchar = emscripten_atomic_add_u16(&globalUshort, 1); assert(prevUshort == 4);
	uint32_t prevUchar = emscripten_atomic_add_u32(&globalUint, 1); assert(prevUint == 4);
