...
void *ThreadMain(void *arg)
{
	...
	THREAD_LOCAL_EM_ASM_INT( { Module['print']('Thread ' + $0 + ' for test ' + $1 + ': starting computation.'); }, t->threadId, t->op);
	...
	THREAD_LOCAL_EM_ASM_INT( { Module['print']('Thread ' + $0 + ' for test ' + $1 + ': finished, exit()ing.'); }, t->threadId, t->op);
	pthread_exit(0);
}
...
{	
	...
	THREAD_LOCAL_EM_ASM_INT( { Module['print']('Main: Starting test ' + $0); }, test);
	...
	THREAD_LOCAL_EM_ASM_INT( { Module['print']('Main: Test ' + $0 + ' finished. Result: ' + $1); }, test, val);
	if (test != 6)
	{
		for(int i = 1; i < N; ++i)
...
#else
	THREAD_LOCAL_EM_ASM(Module['print']('Main: Test successfully finished.'));
#endif
}
