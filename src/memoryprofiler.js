...
/*
    ...
    if () {
      ...
      if (this.allocatedPointersPerCallstack[caller] > 0) ++this.allocatedPointersPerCallstack[caller];
      else this.allocatedPointersPerCallstack[caller] = 1;
    }
  */
  },
 ...
 filterCallstack: function(callstack) {
  ...
  updateUi: function updateUi() {
    ...
    html += "<br />" + colorBar("#6699CC") + colorBar("#003366") + colorBar("#0000FF") + "DYNAMIC memory area used: " + this.formatBytes(this.totalMemoryAllocated) + " (" + (this.totalMemoryAllocated * 100 / (TOTAL_MEMORY - DYNAMIC_BASE)).toFixed(2) + "% of all dynamic memory)";
    html += "<br />" + colorBar("#70FF70") + "DYNAMIC memory area free: " + this.formatBytes(DYNAMICTOP - DYNAMIC_BASE - this.totalMemoryAllocated) + " (" + ((DYNAMICTOP - DYNAMIC_BASE - this.totalMemoryAllocated) * 100 / (TOTAL_MEMORY - DYNAMIC_BASE)).toFixed(2) + "% of all dynamic memory)";
    ...
    if (Object.keys(this.allocationsAtLoc).length > 0) {
      ...
      if (calls.length > 0) {
        ...
        for (var i in calls) {
          html += "<b>" + this.formatBytes(calls[i][1]) + '/' + calls[i][0] + " allocs</b>: " + calls[i][2] + "<br />";
        }
      }
    }
