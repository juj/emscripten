var LibraryPThread = {
  ...
  pthread_create: function(pthread_ptr, attr, start_routine, arg) {
    ...
    if (attr) {
      ...
      // Musl has a convention that the stack size that is stored to the pthread attribute structure is always DEFAULT_STACK_SIZE smaller than the
      // actual used stack size. That is, stored stack size of 0 would mean a stack of DEFAULT_STACK_SIZE in size. All musl functions hide this impl
      // detail, and offset the size transparently, so pthread_*() API user does not operate in these offset sizes. When reading the structure directly,
      // we need to offset the size manually here.
