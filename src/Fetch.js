...
function emscripten_start_fetch(fetch, successcb, errorcb, progresscb) {
  ...
  if (!fetchAttrReplace || requestMethod === 'EM_IDB_STORE' || requestMethod === 'EM_IDB_DELETE') {
    ...
    } else if (requestMethod === 'EM_IDB_DELETE') __emscripten_fetch_delete_cached_data(Fetch.dbInstance, fetch, reportSuccess, reportError);
  ...
  if (!fetchAttrNoDownload) __emscripten_fetch_xhr(fetch, fetchAttrPersistFile ? cacheResultAndReportSuccess : reportSuccess, reportError, reportProgress);
#if FETCH_DEBUG
